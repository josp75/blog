        ...
        <div class="relative flex lg:inline-flex items-center bg-gray-100 rounded-xl">
            <div x-data="{ show: false }" @click.away="show = false">
                <button @click="show = !show"
                        class="py-2 pl-3 pr-9 text-sm font-semibold w-full lg:w-32 text-left flex lg:iniline-flex">
                    {{  isset($currentCategory) ? $currentCategory->title : 'Categories'}}
            ...
            <div x-show="show" class="py-2 absolute bg-gray-100 w-full mt-2 rounded-xl z-50" style="display: none">
                ...
                @foreach($categories as $category)
                    <a href="categories/{{$category->slug}}"
                       class="block text-left px-3 text-sm leading-6 hover:bg-blue-500  {{isset($currentCategory) && $currentCategory->is($category)?  'bg-blue-500 text-white' : ''}}  focus:bg-blue-500 hover:text-white  focus:text-white">
                        {{ ucwords($category->title)}}</a>
